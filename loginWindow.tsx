...
export default function LoginWindow(
  ...
  /* 
    #1: Add a state variable to store the current error message
  */
 const[error, setError ] = useState<string>("");
  ...
  /*
    #2: This function is called to set up the profile of the new user. It is retreived
  ...
  */
  const onSubmit = async (form: FormData) => {
    /* 
      #3: Set the error state to an empty string.
    */
   setError("");
    /* 
      #4: Set up a try catch block to call the setUpProfile() function and set the error state
    ...
    */
   try{
    await setUpProfile(form); //call the setupProfile() function.
   }catch(error: any){
      ...
      <div className='flex flex-col space-y-3'>
        ...
        <div className="w-full">
          ...
          <p className="text-red-500">
            {/* 
              #2: Display the error message if it is not an empty string using the error state variable.
            */}
            {error}
          </p>
        </div>
